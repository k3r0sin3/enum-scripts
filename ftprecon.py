...
FTPSCAN = "nmap -sV -Pn -vv -p %s --script=ftp-anon,ftp-bounce,ftp-libopie,ftp-proftpd-backdoor,ftp-vsftpd-backdoor,ftp-vuln-cve2010-4221,tftp-enum.nse -oN '/root/scripts/recon_enum/results/exam/%s_ftp.nmap' %s" % (port, ip_address, ip_address)

results = subprocess.check_output(FTPSCAN, shell=True)
outfile = "/root/recon_scan/results/" + ip_address + "_ftprecon.txt"
f = open(outfile, "w")
f.write(results)
f.close
