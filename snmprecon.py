...
if results != "":
    ...
    if snmpdetect == 1:
        ...
        SNMPWALK = "snmpwalk -c public -v1 %s 1 > /root/results/%s_snmpwalk.txt" % (ip_address, ip_address)
        results = subprocess.check_output(SNMPWALK, shell=True)

NMAPSCAN = "nmap -vv -sV -sU -Pn -p 161,162 --script=snmp-netstat,snmp-processes,snmp-info,snmp-brute %s" % (ip_address)
results = subprocess.check_output(NMAPSCAN, shell=True)
resultsfile = "/root/recon_scan/results/" + ip_address + "_snmprecon.txt"
